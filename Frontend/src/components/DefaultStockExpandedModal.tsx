        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-3">
            {['day', 'month', 'year'].map((tf) => (
              <button key={tf} onClick={async (e) => { e.stopPropagation(); const newTimeframe = tf as 'day' | 'month' | 'year'; setTimeframe(newTimeframe); await fetchStockDetail(stock.symbol) }} className={`flex-1 py-3 px-6 rounded-lg text-base font-medium transition-colors ${timeframe === tf ? 'bg-[#9DB38A] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
