  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-3 border border-gray-300 rounded-lg shadow-lg">
        ...
        <p className="text-sm font-bold text-gray-900">${formatNumber(data.price)}</p>
        {data.type === 'forecast' && (
...
export function StockCardStack({
  ...
  const getCardStyle = (stockIndex: number) => {
    ...
    else if (relativePosition < -stocks.length / 2) relativePosition += stocks.length

    const distance = Math.abs(relativePosition)
    if (distance === 0) return { transform: 'translateX(0) scale(1)', zIndex: 30, filter: 'blur(0px)', opacity: 1 }
    if (distance === 1) return { transform: `translateX(${relativePosition > 0 ? '180px' : '-180px'}) scale(0.85)`, zIndex: 20, filter: 'blur(2px)', opacity: 0.6 }
    if (distance === 2) return { transform: `translateX(${relativePosition > 0 ? '300px' : '-300px'}) scale(0.7)`, zIndex: 10, filter: 'blur(4px)', opacity: 0.3 }
  ...
  // Calculate Y-axis domain for current stock
  const yAxisDomain = useMemo(() => {
    ...
    const historicalData = combinedChartData.filter(d => d.type !== 'forecast')
    ...
    console.log('📊 Y-axis domain calculated:', domain, 'for', combinedChartData.length, 'points')
    
    if (combinedChartData.length > 0) {
      const prices = combinedChartData.map(d => d.price).filter(p => p != null)
      console.log('💰 Price range in combined data:', Math.min(...prices), 'to', Math.max(...prices))
      
      if (forecastData.length > 0) {
        const forecastPrices = forecastData.map(d => d.price).filter(p => p != null)
        console.log('🔮 Forecast price range:', Math.min(...forecastPrices), 'to', Math.max(...forecastPrices))
      }
    }
    return domain
  }, [combinedChartData])
    ...
    <>
              ...
              <div className="w-full h-full rounded-lg bg-white shadow-sm p-6 flex flex-col gap-4 border border-gray-200 hover:border-[#9DB38A] transition-all duration-300">
                ...
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-gray-900 text-3xl font-bold">${formatNumber(stock.price)}</p>
                ...
                <div className="flex-shrink-0">
                  <div className="w-full h-64 rounded-lg bg-gray-50 border-2 border-gray-200 overflow-hidden">
                    {index === currentIndex && (
                      <div className="absolute top-2 left-2 text-xs bg-white/80 p-1 rounded z-10">
                        H:{combinedChartData.filter(d => d.type !== 'forecast').length} 
                        F:{combinedChartData.filter(d => d.type === 'forecast').length}
                      </div>
                    )}
                  ...
                  {/* Legend */}
                  {index === currentIndex && combinedChartData.some(d => d.type === 'forecast') && (
                ...
                <div className="grid grid-cols-5 gap-3 h-[240px]">
                  <div className="col-span-2 grid grid-rows-2 gap-3">
                    <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between text-xs"><span className="text-gray-600">Open:</span><span className="font-medium text-gray-900">${formatNumber(currentDetail?.open)}</span></div>
                        <div className="flex justify-between text-xs"><span className="text-gray-600">High:</span><span className="font-medium text-gray-900">${formatNumber(currentDetail?.high)}</span></div>
                        <div className="flex justify-between text-xs"><span className="text-gray-600">Low:</span><span className="font-medium text-gray-900">${formatNumber(currentDetail?.low)}</span></div>
                      </div>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                      <p className="text-xs font-semibold text-gray-500 uppercase mb-1">Volume</p>
                      <p className="text-lg font-bold text-gray-900">{formatVolume(currentDetail?.volume)}</p>
                      <p className="text-xs text-gray-600">Avg: {formatVolume(currentDetail?.avgVolume)}</p>
                    </div>
                  </div>
                  ...
                  <div className="col-span-2 grid grid-rows-2 gap-3">
                    <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                      ...
                      <p className="text-xs text-gray-900 font-medium">${formatNumber(currentDetail?.fiftyTwoWeekLow)} - ${formatNumber(currentDetail?.fiftyTwoWeekHigh)}</p>
                      <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                        {currentDetail?.fiftyTwoWeekLow != null && currentDetail?.fiftyTwoWeekHigh != null && (
                          <div className="bg-[#9DB38A] h-2 rounded-full" style={{ width: `${((stock.price - currentDetail.fiftyTwoWeekLow) / (currentDetail.fiftyTwoWeekHigh - currentDetail.fiftyTwoWeekLow)) * 100}%` }} />
                        )}
                      </div>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between text-xs"><span className="text-gray-600">P/E:</span><span className="font-medium text-gray-900">{formatNumber(currentDetail?.peRatio)}</span></div>
                        <div className="flex justify-between text-xs"><span className="text-gray-600">Sector:</span><span className="font-medium text-gray-900 truncate ml-2">{currentDetail?.sector || '-'}</span></div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="col-span-1 bg-gradient-to-br from-[#eff3eb] to-blue-50 rounded-lg p-3 border-2 border-[#9DB38A] flex flex-col justify-between">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-900">${formatNumber(stock.price * (1 + Math.abs(stock.changePercent) / 100))}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {expandedStock && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-8" onClick={closeExpanded}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] p-8 relative overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <button onClick={closeExpanded} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700">✕</button>
            <h2 className="text-3xl font-bold mb-4">{expandedStock.symbol} - {expandedStock.company}</h2>
